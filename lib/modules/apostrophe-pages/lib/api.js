...
module.exports = function(self, options) {
    ...
    return async.series(
      [
        ...
        function getChildren(cb) {
          ...
          self.find(req, { path: path }).toArray((err, res) => {
            if (!err) {
        ...
        function trashOrUntrashPages(cb) {
          ...
          return self.apos.docs.db.update({ _id: { $in: ids } }, action, (err, res) => {
            cb(err);
          });
        },
        ...
        function dedupePages(cb) {
          return self.dedupePages(req, tree, toTrash, cb);
        }
      ], err => {
  ...
  self.dedupePages = function(req, pages, toTrash, callback) {
    return async.eachSeries(pages, (page, cb) => {
      if (toTrash) {
        return self.apos.docs.getManager(page.type).deduplicateTrash(req, page, cb);
      } else {
        return self.apos.docs.getManager(page.type).deduplicateRescue(req, page, cb);
      }
    }, callback);
  };
