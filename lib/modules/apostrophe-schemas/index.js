...
module.exports = {
  ...
  construct: function(self, options) {
    ...
    var browserOptions = {
      action: self.action,
      messages: {
        required: self.apos.i18n.__('Required'),
        invalid: self.apos.i18n.__('Value is not valid'),
        min: self.apos.i18n.__('Min. permitted value is: %s'),
        max: self.apos.i18n.__('Max. permitted value is: %s'),
        minString: self.apos.i18n.__('Minimum of %s characters'),
        maxString: self.apos.i18n.__('Maximum of %s characters')
      }
    };

    self.pushCreateSingleton = function() {
      self.apos.push.browserCall('user', 'apos.create("apostrophe-schemas", ?)', browserOptions);
    };
        ...
        _.each(errors, function(err, name) {
          if ((typeof err) === 'string') {
            if ((err === 'required') && (!self.isVisible(schema, object, name))) {
              ...
              self.apos.utils.warn('Field ' + name + ' reported a validation error for the value ' + JSON.stringify(data) + ', tolerant sanitization in effect, delivering default');
    ...
    self.isVisible = function(schema, object, name) {
      ...
      _.each(schema, function(field) {
        ...
        _.each(field.choices, function(choice) {
          if (choice.showFields && (object[field.name] !== choice.value)) {
