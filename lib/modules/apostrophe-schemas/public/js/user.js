...
apos.define('apostrophe-schemas', {
  ...
  construct: function(self, options) {
    ...
    self.error = function(field, type) {
      ...
      switch (type) {
        case 'required':
          error.message = _.get(options.messages, 'required', 'required');
          break;
        case 'min':
          error.message = _.get(options.messages, 'min', 'min').replace('%s', field.min);
          break;
        case 'max':
          error.message = _.get(options.messages, 'max', 'max').replace('%s', field.max);
          break;
        case 'invalid':
          error.message = _.get(options.messages, 'invalid', 'Invalid');
          break;
      }
      return error;
    };
    ...
    function convertString(data, name, $field, $el, field, callback) {
      ...
      if (data[name] && field.max && (data[name].length > field.max)) {
        ...
        err.message = options.messages.maxString.replace('%s', field.max);
        return setImmediate(_.partial(callback, err));
      }
      if (data[name] && field.min && (data[name].length < field.min)) {
        err = self.error(field, 'min');
        err.message = options.messages.minString.replace('%s', field.min);
        return setImmediate(_.partial(callback, err));
      }
      return setImmediate(callback);
    }
    ...
    self.addFieldType({
      ...
      populate: function(data, name, $field, $el, field, callback) {
        ...
        if (field.legacy) {
          apos.ui.enhanceDate(self.findField($el, field.legacy), options);
        }
        return setImmediate(callback);
      },
      convert: convertString
